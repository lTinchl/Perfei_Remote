...
void Timer_Init(void)
{
	...
	/*?????*/
	TIM_InternalClockConfig(TIM3);		//??TIM3??????????????TIM????????
	...
	TIM_TimeBaseInit(TIM3, &TIM_TimeBaseInitStructure);				//????????TIM_TimeBaseInit???TIM4?????	
	...
	TIM_ITConfig(TIM3, TIM_IT_Update, ENABLE);					//??TIM4?????
	...
	NVIC_InitStructure.NVIC_IRQChannel = TIM3_IRQn;				//????NVIC?TIM4?
